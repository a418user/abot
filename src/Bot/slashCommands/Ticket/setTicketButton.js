...
module.exports = class TicketButton extends SlashCommand {
  constructor(handler) {
    super(handler, {
      name: "ticket-bouton",
      description: "Gérer le système de tickets par bouton",
      options: [
        {
          name: "config",
          description: "Configurer le système de tickets par bouton",
          type: 1,
        },
        {
          name: "send-menu",
          description: "Envoyer l'embed pour ouvrir un ticket par bouton",
          type: 1,
          options: [
            {
              name: "id",
              description:
                "Choisir l'identifiant de la catégorie de tickets à envoyer",
              type: 3,
              required: true,
            },
            {
  ...
  displayEmbedAndComponents(ctx, allDatabase, ticketData) {
    ...
    if (!ticketData) {
      categoryChannelValue = ctx.translate`Tickets ouverts ➣ ${ctx.emojiError}`;
      categoryChannelCloseValue = ctx.translate`Tickets fermés ➣ ${ctx.emojiError}`;
      ...
      msgTicketValue = ctx.translate`Mention ➣ ${ctx.emojiError}\nMessage ➣ ${ctx.emojiError}`;
      numberTicketValue = "`0`";
      type = `${ctx.emojiError}`;
      ...
      const openedCategory = ticketData.category_id
        ? ctx.getChannel(ticketData.category_id)
        : null;
      const closedCategory = ticketData.category_close_id
        ? ctx.getChannel(ticketData.category_close_id)
        : null;

      categoryChannelValue = ticketData.category_id
        ? openedCategory
          ? ctx.translate`Tickets ouverts ➣ ${openedCategory.name.toUpperCase()}`
          : ctx.translate`⚠️ Tickets ouverts ➣ \`${ticketData.category_id}\``
        : ctx.translate`Tickets ouverts ➣ ${ctx.emojiError}`;
      categoryChannelCloseValue = ticketData.category_close_id
        ? closedCategory
          ? ctx.translate`Tickets fermés ➣ ${closedCategory.name.toUpperCase()}`
          : ctx.translate`⚠️ Tickets fermés ➣ \`${ticketData.category_close_id}\``
        : ctx.translate`Tickets fermés ➣ ${ctx.emojiError}`;
      nameTicketValue = ticketData.name_ticket
        ? ctx.translate`\`${ticketData.name_ticket}\``
        : `${ctx.emojiError}`;
      ...
      const msgValue = ticketData.msg_ticket
        ? ctx.translate`\`${ticketData.msg_ticket}\``
        : ctx.translate`Message ➣ ${ctx.emojiError}`;
      if (ticketData.roles_mention) {
        for (const roleId of JSON.parse(ticketData.roles_mention)) {
          const role = ctx.getRole(roleId);

          if (rolesMentionValue.length === 0) {
            if (role) rolesMentionValue.push(ctx.translate`Mention ➣ ${role}`);
            else
              rolesMentionValue.push(ctx.translate`Mention ➣ ⚠️ \`${roleId}\``);
          } else {
            if (role) rolesMentionValue.push(role);
            else rolesMentionValue.push(ctx.translate`⚠️ \`${roleId}\``);
          }
        }
      }

      if (rolesMentionValue.length === 0)
        rolesMentionValue.push(ctx.translate`Mention ➣ ${ctx.emojiError}`);
    ...
    const embed = new EmbedBuilder()
      .setAuthor({
        name: ctx.translate`\uD83C\uDFAB Configuration du système de tickets \uD83C\uDFAB`,
        iconURL: `${ctx.guild.iconURL() || ctx.client.user.displayAvatarURL()}`,
      })
      .setColor(ctx.colors.blue)
      .setDescription(ctx.translate`Id : ${type}`)
      .setFooter({
        text: ctx.translate`✅ Configuré | ⚠️ Problème détecté | ❌ Non configuré`,
      })
      .addFields([
        {
          name: ctx.translate`1️⃣ Catégories`,
          value: `${categoryChannelValue}\n${categoryChannelCloseValue}`,
          inline: true,
        },
        {
          name: ctx.translate`2️⃣ Nom`,
          value: `${nameTicketValue}`,
          inline: true,
        },
        { name: `\u200b`, value: `\u200b`, inline: true },
        {
          name: ctx.translate`3️⃣ Rôles ayant accès aux tickets`,
          value: `${roleTicketValue}`,
          inline: true,
        },
        {
          name: ctx.translate`4️⃣ Salon des logs`,
          value: `${logTicketValue}`,
          inline: true,
        },
        { name: `\u200b`, value: `\u200b`, inline: true },
        {
          name: ctx.translate`5️⃣ Salon des archives`,
          value: `${archiveTicketValue}`,
          inline: true,
        },
        {
          name: ctx.translate`6️⃣ Message envoyé lors de l'ouverture d'un ticket`,
          value: `${msgTicketValue}`,
          inline: true,
        },
        { name: `\u200b`, value: `\u200b`, inline: true },
        {
          name: ctx.translate`7️⃣ Embed pour ouvrir un ticket`,
          value: `${embedTicketValue}`,
          inline: true,
        },
        {
          name: ctx.translate`8️⃣ Compteur des tickets`,
    ...
    menu
      .addOptions({
        label: ctx.translate`Nouvelle catégorie`,
        emoji: "🆕",
        value: "new_type",
        description: ctx.translate`Créer une nouvelle catégorie`,
      })
      .addOptions({
        label: ctx.translate`Supprimer catégorie`,
        emoji: "🚨",
        value: "remove_type",
        description: ctx.translate`Supprimer une catégorie`,
      })
      .addOptions({
        label: ctx.translate`Vide`,
        emoji: "💤",
        value: "nothing_type",
        description: ctx.translate`Cliquer ici pour ne rien faire`,
      });
  ...
  async run(ctx) {
    ...
    if (subCommand === "config") {
      ...
      collectorMenu.on("collect", async (menu) => {
        ...
        if (choice === "new_type") {
          ...
          const modal = new ModalBuilder()
            ...
            .setTitle(ctx.translate`Création d'une nouvelle catégorie`);

          const textInput = new TextInputBuilder()
            .setCustomId("modal_new_ticket_name")
            .setStyle(1)
            .setLabel(ctx.translate`Quel est le nom de la nouvelle catégorie ?`)
            .setRequired(true)
            .setMaxLength(50);
          ...
          ctx.inter
            ...
            .then(async (modal) => {
              ...
              for (let i = 0; i < baseTicket.length; i++) {
                if (baseTicket[i].choice_type === name.toLowerCase()) {
                  return modal.followUp({
                    content: ctx.translate`${ctx.emojiError} Le nom de catégorie **${name}** existe déjà !`,
                    flags: 64,
                  });
                }
              }
              ...
              if (baseTicket.length >= 20)
                return modal.followUp({
                  content: ctx.translate`${ctx.emojiError} Vous ne pouvez pas créer plus de catégorie !`,
                  flags: 64,
                });

              await ctx.database.table("guild_ticket").insert({
          ...
          if (!baseTicket[0])
            return menu.followUp({
              content: ctx.translate`${ctx.emojiError} Il n'existe aucune catégorie !`,
              flags: 64,
            });
          ...
          const embed = new EmbedBuilder()
            .setDescription(
              ctx.translate`**Quelle catégorie souhaitez-vous supprimer ?**`
            )
            .setColor(ctx.colors.blue);

          const menuType = new StringSelectMenuBuilder()
            .setPlaceholder(ctx.translate`Sélectionner une catégorie`)
            .setMinValues(1)
            .setMaxValues(1)
          ...
          collectorRemoveType.on("collect", async (menuType) => {
            ...
            if (choiceType !== "cancel_delete_type") {
              ...
              if (checkIfHasTable)
                await ctx.database.table("guild_ticket").delete().where({
                  guild_id: ctx.guild.id,
      ...
      collector.on("collect", async (button) => {
        if (button.customId === "category") {
          ...
          const embedChoice = new EmbedBuilder()
            .setDescription(
              ctx.translate`**Quelle action souhaitez-vous effectuer ?**`
            )
            .setColor(ctx.colors.blue);

          const buttonCategory = new ButtonBuilder()
            .setStyle(2)
            .setLabel(ctx.translate`Tickets ouverts`)
            .setCustomId("category_ticket")
            .setEmoji("📥");

          const buttonCategoryClose = new ButtonBuilder()
            .setStyle(2)
            .setLabel(ctx.translate`Tickets fermés`)
          ...
          collectorQuestion.on("collect", async (buttonQuestion) => {
            ...
            if (buttonQuestion.customId === "category_ticket") {
              const displayEmbed = new EmbedBuilder()
                .setDescription(
                  ctx.translate`**Choisir une catégorie où seront envoyés les tickets ouverts**`
                )
                .setColor(ctx.colors.blue);

              const channelSelectMenu = new ChannelSelectMenuBuilder()
                .setCustomId("category")
                .setPlaceholder(ctx.translate`Catégorie du serveur`)
                .addChannelTypes(4)
                .setMinValues(0)
                .setMaxValues(1);
              ...
              const displayEmbed = new EmbedBuilder()
                .setDescription(
                  ctx.translate`**Choisir une catégorie où seront envoyés les tickets fermés**`
                )
                .setColor(ctx.colors.blue);

              const channelSelectMenu = new ChannelSelectMenuBuilder()
                .setCustomId("category")
                .setPlaceholder(ctx.translate`Salons du serveur`)
                .addChannelTypes(4)
                .setMinValues(0)
                .setMaxValues(1);
          ...
          const modal = new ModalBuilder()
            ...
            .setTitle(ctx.translate`Choix du nom des tickets`);

          const textInput = new TextInputBuilder()
            .setCustomId("modal_name_ticket")
            .setStyle(2)
            .setLabel(ctx.translate`Quel est le nom donné aux tickets ?`)
            .setPlaceholder(
              ctx.translate`Mettre "author" pour avoir le nom de l'auteur du ticket`
            )
            .setRequired(false)
            .setMaxLength(90);
          ...
          const displayEmbed = new EmbedBuilder()
            .setDescription(
              ctx.translate`**Choisir des rôles à ajouter aux tickets**`
            )
            .setColor(ctx.colors.blue);

          const roleSelectMenu = new RoleSelectMenuBuilder()
            .setCustomId("role_ticket")
            .setPlaceholder(ctx.translate`Rôles du serveur`)
            .setMinValues(0)
            .setMaxValues(10);
          ...
          const displayEmbed = new EmbedBuilder()
            .setDescription(
              ctx.translate`**Choisir le salon où seront envoyés les logs des tickets**`
            )
            .setColor(ctx.colors.blue);
          ...
          const displayEmbed = new EmbedBuilder()
            .setDescription(
              ctx.translate`**Choisir le salon où seront envoyés les archives des tickets**`
            )
            .setColor(ctx.colors.blue);
          ...
          const buttonMention = new ButtonBuilder()
            .setStyle(2)
            .setLabel(ctx.translate`Mentionner des rôles`)
            .setCustomId("mention_ticket")
            .setEmoji("📢");

          const buttonMessage = new ButtonBuilder()
            .setStyle(2)
            .setLabel(ctx.translate`Écrire un message`)
          ...
          collectorQuestion.on("collect", async (buttonQuestion) => {
            ...
            if (buttonQuestion.customId === "message_ticket") {
              ...
              const modal = new ModalBuilder()
                ...
                .setTitle(ctx.translate`Message d'ouverture d'un ticket`);

              const textInput = new TextInputBuilder()
                .setCustomId("modal_msg_ticket")
                .setStyle(2)
                .setLabel(ctx.translate`Quel message doit être envoyé ?`)
                .setRequired(false)
                .setMaxLength(300);
              ...
              const displayEmbed = new EmbedBuilder()
                .setDescription(
                  ctx.translate`**Choisir les rôles à mentionner lors de l'ouverture d'un ticket**`
                )
                .setColor(ctx.colors.blue);
          ...
          collectorEmbed.on("collect", async (button) => {
            if (button.customId === "pencil2") {
              ...
              const modal = new ModalBuilder()
                ...
                .setTitle(ctx.translate`Miniature`);

              const textInput = new TextInputBuilder()
                .setCustomId("modal_thumbnail_embed")
                .setStyle(2)
                .setLabel(ctx.translate`Quel est le lien de la miniature ?`)
                .setRequired(false);
      ...
      const type = ctx.options.getString("id");
      ...
      const baseTicket = await ctx.database
        .table("guild_ticket")
        .select()
        .where({ guild_id: ctx.guild.id, choice_type: type.toLowerCase() });
      if (!baseTicket[0])
        return ctx.send({
          content: ctx.translate`${ctx.emojiError} Cette catégorie de tickets n'existe pas !`,
          flags: 64,
        });
      if (!baseTicket[0].embed_ticket)
        return ctx.send({
      ...
      const openTicket = new ButtonBuilder()
        ...
        .setCustomId(`create_ticket_${type.toLowerCase()}`);
