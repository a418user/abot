...
module.exports = class CronStatisticsOwner {
    constructor(event) {
    ...
    async channels() {
        const guildStatistics = await this.database.table('guild_statistics_owner').select();
        ...
        for (const statistics of guildStatistics) {
            ...
            if (channel1 && !channel1.name.includes(String(humans.size))) {
                channel1.setName(`${statistics.channel1_name?statistics.channel1_name:"Membres :"} ${humans.size}`).catch(() => null);
            }
            if (channel2 && !channel2.name.includes(String(bots.size))) {
                channel2.setName(`${statistics.channel2_name?statistics.channel2_name:"Serveurs :"} ${guilds}`).catch(() => null);
            }
            if (channel3 && !channel3.name.includes(String(members.size))) {
                channel3.setName(`${statistics.channel3_name?statistics.channel3_name:"Bots :"} ${bots.size}`).catch(() => null);
            }
        }
    }
